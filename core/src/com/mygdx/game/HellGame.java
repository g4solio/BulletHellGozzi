...
public class HellGame extends ApplicationAdapter {
	...
	public void create() {
		...
		spaceShip.x = 800 / 2 - 64 / 2; // center the spaceShip horizontally
	...
	private void spawnRaindrop() {
		new Projectile(new Rectangle(MathUtils.random(0, 800-64),480,64,64), projectileTexture,270,MathUtils.random(10,300));
	...
	public void render() {
		...
		for(Projectile projectile: projectileArray) {
			batch.draw(projectileTexture, projectile.projectile.x, projectile.projectile.y,64,64);
		}
		batch.end();
		...
		if(spaceShip.y > 480 - 64) spaceShip.y = 480 - 64;
		...
		while(iter.hasNext()) {
			...
			Gdx.app.log("Gocciacos",Math.round(Math.cos(projectile.degree)) + "");
			Gdx.app.log("Gocciasin",Math.round(Math.sin(projectile.degree)) + "");
